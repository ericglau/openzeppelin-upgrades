...
export class Manifest {
  ...
  private async lock() {
    // debug("DEVDIR " + this.devDir);
    // debug("DEVINST " + JSON.stringify(this.devInstanceMetadata, null, 2));
    const lockfileName = path.join(
      this.devDir ?? MANIFEST_DEFAULT_DIR,
      `chain-${getSuffix(this.chainId, this.devInstanceMetadata)}`,
    );

    debug("lockfile " + lockfileName);
    debug("manifest " +  this.file + " exists ? " + await this.exists(this.file));
