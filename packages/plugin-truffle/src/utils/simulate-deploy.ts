...
export async function simulateDeployAdmin(ProxyAdminFactory: ContractClass, opts: Options, adminAddress: string, adminBytecode: string) {
...
export async function simulateDeployImpl(Contract: ContractClass, opts: Options, implAddress: string, runtimeBytecode: string) {
...
async function simulateDeployment(
  ...
  const simulateDeploy = await getSimulateDeploy(deployData, Contract, implAddress, runtimeBytecode && hashBytecode(runtimeBytecode));
