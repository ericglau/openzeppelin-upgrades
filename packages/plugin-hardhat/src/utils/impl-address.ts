 ...
 */
export async function getImplementationAddressFromBeacon(hre: HardhatRuntimeEnvironment, signer: ethers.Signer | undefined, beaconAddress: string) {
 ...
 */
export async function getImplementationAddressFromProxy(provider: EthereumProvider, proxyAddress: string, hre: HardhatRuntimeEnvironment, proxy: Contract | ContractAddressOrInstance, signer: Signer | undefined) {
  ...
  try {
    ...
    try {
      ...
      result = await getImplementationAddressFromBeacon(hre, proxy instanceof Contract ? proxy.signer : signer, beaconAddress);      
    } catch (e: any) {
 ...
 * 
 * @returns a Promise with the interface, or undefined if implAddress is undefined or the implementation interface cannot be found in the manifest.
 */
export async function getInterfaceFromManifest(hre: HardhatRuntimeEnvironment, implAddress?: string) : Promise<utils.Interface | undefined> {
