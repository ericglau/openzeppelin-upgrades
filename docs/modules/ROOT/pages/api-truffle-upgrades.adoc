...
Imports an existing proxy deployment to be used with this plugin. Provide the address of an existing proxy and the Truffle contract class of the implementation contract that was deployed. This can be used to recreate a lost https://docs.openzeppelin.com/upgrades-plugins/1.x/network-files[network file] by importing previously deployed proxies, or to register proxies for upgrading even if they were not originally deployed by this plugin. Supported for UUPS, Transparent, and Beacon proxies.

* `force`: whether to force the import even if the runtime bytecode at the implementation address does not correspond to the provided Truffle contract class.
