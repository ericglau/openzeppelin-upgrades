...
Validates an implementation contract.
...
Validates a new implementation contract and compares with the original implementation contract (given a proxy or beacon address that uses the original implementation, or an ethers contract factory corresponding to the original implementation) to check for storage layout compatibility errors.
...
Validate upgrade for an existing proxy to a new contract:
[source,ts]
----
...
Validate upgrade between two contract implementations:
[source,ts]
----
